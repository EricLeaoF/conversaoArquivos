...
export async function convertPDF2JPG(){
    ...
    const pathRede = `//10.150.0.28/Movida_Multas/Planilhas Indicação/ConverterArquivos/PDF-JPG/Converter`
    const pathSaida = `//10.150.0.28/Movida_Multas/Planilhas Indicação/ConverterArquivos/PDF-JPG/Convertido`
    ...
    for(let i = 0; i<foldersJPG.length;i++){
        ...
        for(let arquivo of arquivos){
            ...
            const convertPDF:any = await pdfToJpg(arquivo, pathLocal);
